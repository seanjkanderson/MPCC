function [state_pred, input_pred, sol, exitflag, CMPC] = cautiousMPCControllerLinear(stage, gp, mpc_vars, ModelParams, previous_x, learned)
...
%     dyn = [];
    ...
    for k = 1:mpc_vars.N
        ...
        u(:,k)'*Ru(:,:,k)*u(:,k)) + f(:,k)'*x(:,k) + s(k)^2*1e4 + s(k)*1e4;
...
%         else
        ...
        constraints = [constraints,
        x(:, k+1) == A(:,:,k)*x(:,k) + B(:,:,k)*u(:,k) + g(:,k) + s(k)];
%         end

        constraints = [constraints;
                       lb(:,k) <= [x(:,k);u(:,k)] <= ub(:,k); % bounds
                        lg(1,k) <= C(1,:,k)*x(:,k) <= ug(1,k) % track
                       ]; 

    end
    % Add terminal cost: let terminal_cost = f(mu_ix, mu_iu, sigma_ix)
    k = N+1;
    objective = objective + 0.5*(x(:,k)'*Q(:,:,k)*x(:,k)) + f(:,k)'*x(:,k);
...
%     CMPC = optimizer(constraints, objective, options, {x0, Q,Ru,f, A, B, g, lb, ub, lg, ug, C}, {x, u});
    params = {x0,Q,Ru,f, A, B, g, lb, ub, lg, ug, C};
    CMPC = optimizer(constraints, objective, options, params, {x, u});
    toc;
else
...
% Basic way to test if the problem is feasible is by making sure the sum is
% non-zero. For infeasible problems this is a zero matrix.
if sum(sum(sol{1})) > 0
    exitflag = 0;
else
    exitflag = 1;
