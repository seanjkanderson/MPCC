...
if CarModel == "ORCA"
    ...
    % Added for GP-based MPC (SA)
%     MPC_vars.p_original = .3945; % In the paper, they just let Chi-square(p) = 1. 
    MPC_vars.p_original = .1;
    % TODO: this should not include the auxiliary state AFAIK (SA)
%     MPC_vars.Bd = [zeros(3, 3); eye(3)];
    MPC_vars.Bd = eye(6);
...
%                          3, 3, 10,   4, 2, 7,  30,     1, 0.35,5  ,   1 , 1, 5]'; 
    ...
    % Cost Parameters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    MPC_vars.qC = .1; % contouring cost: default 0.1
    MPC_vars.qCNmult= 10; % increase of terminal contouring cost: default 10
    MPC_vars.qL= 1000; % lag cost
    MPC_vars.qVtheta= 0.02; % theta maximization cost: default 0.02
...
%     MPC_vars.qC = 0; % contouring cost
%     MPC_vars.qCNmult= 0; % increase of terminal contouring cost
%     MPC_vars.qL= 0; % lag cost
%     MPC_vars.qVtheta= 0.02; % theta maximization cost
%     MPC_vars.qOmega = 0; % yaw rate regularization cost
%     MPC_vars.qOmegaNmult = 0; % yaw rate regularization cost
% 
%     MPC_vars.rD= 0; % cost on duty cycle (only used as regularization terms)
%     MPC_vars.rDelta= 0; % cost on steering 
%     MPC_vars.rVtheta= 0; % cost on virtual velocity
% 
%     MPC_vars.rdD= 0.0; % cost on change of duty cycle
%     MPC_vars.rdDelta= 0; % cost on change of steering
%     MPC_vars.rdVtheta= 0.00; % cost on change of virtual velocity
% 
% 
